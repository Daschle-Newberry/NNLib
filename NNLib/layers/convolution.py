...
def old_correlate(x: np.ndarray, kernels: np.ndarray, mode: str, per_batch_kernel : bool):
    """
    :param per_batch_kernel: true if kernel is a batch if kernels. In this case, each batch will use the corresponding batch's kernel
    :param mode : string representing desired convolution mode
    :param x: (batch_size, channels_in, input_height, input_width)
    :param kernels: (*batch_size, channels_out, channels_in, kernel_height, kernel_width)
    ...
    """
    Case 1:
        Input is a batch of several 3D arrays, kernels is a 3D array

            -Create windows of each batch, (batch_size, channels_in, output_height, output_width, kernel_height, kernel_width)
            -Each window in channel i is multiplied and summed over by the kernel of channel i, done for every kernel
    Case 2:
        Input is a batch of several 3D arrays, kernels is a batch of several 3D arrays

            - Create windows of each batch, (batch_size, channels_in, output_height, output_width, kernel_height, kernel_width)
            - For each batch item, take the dot product of each input window and each kernel for that input channel. 
    """



    if per_batch_kernel:
        einstein_sum = 'bixyhw,bohw->boixy'
    else:
        einstein_sum = 'bixyhw,oihw->boxy'
    ...
    if mode == 'full':
        pH, pW = (kernels.shape[-2] - 1, kernels.shape[-1] - 1)
        x = np.pad(x, pad_width=((0, 0), (0, 0), (pH, pH), (pW, pW)), mode='constant', constant_values=0)
    elif mode == 'valid':
        pass

    windows = np.lib.stride_tricks.sliding_window_view(x, (kernels.shape[-2], kernels.shape[-1]), axis=(2, 3))


    output = np.einsum(einstein_sum, windows, kernels)
